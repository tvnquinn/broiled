...
5. **Rich notifications (Notification Content Extension).** ⏳ *Deliberately skipped for now (July 13) - the only remaining Wave 2 item.* Big expanded layout with large action buttons (iOS constraint: collapsed banners cannot show buttons; actions appear on long-press/expand - the extension makes the expanded state big and custom).
...
6. **Workout types.** ✅ *Shipped July 13.* New `WorkoutEntry` model (dateKey, type, source healthKit/manual, duration, note) supporting **multiple workouts per day**; `DayLog` still owns the day's status. Schedule gained an optional per-day workout type (Apple-Fitness-style menu + custom free text in the schedule editor; "any workout" clears it); the T-30 reminder becomes "30 min till <type>" and the Home label "<type> in"; HealthKit-detected workouts record their real activity type (`HKWorkoutActivityType.displayName`, lowercase to match the voice).
7. **Rest-day flow + bonus workouts.** ✅ *Shipped July 13.* Fixed the real bug (Home rendered a red 0:00 countdown on non-scheduled days because the deadline fell back to `Date()` - it's now a true optional). Proper rest-day state with "log a bonus workout" (HealthKit-checked first, honesty-gate fallback). Bonus workouts get a `.bonus` DayLog status + a WorkoutEntry but never advance the streak: "cute. bonus workouts don't buy back missed ones - no streak freezes here." Onboarding on a rest day no longer manufactures a fake next-day deadline.
8. **Pause mode.** ✅ *Shipped July 13.* Settings → Pause (from/until date pickers): no notifications (everything pending cancelled, nothing re-arms), no misses (reconcile skips the whole range), streak frozen. Home shows a paused state with no action buttons, even on scheduled days. Reconcile clears an expired range and stamps a one-calendar-day resume banner: "break's over. hope you're not." "resume now" ends it early and earns the same line.
